...
const watcher = chokidar.watch('./post/**/*', {
  cwd,
  ignoreInitial: true,
})
  .on('ready', async () => {
  ...
  .on('all', (event, filepath, stats) => {
    switch (event) {
      ...
      case 'unlink':
      case 'unlinkDir':
        fsExtra.remove(memoizedPostDestinationPath(filepath))
          .catch(e => `Error :: unsuccessful unlink for path: ${memoizedPostDestinationPath(filepath)}`);
      break;
    }
  });
...
function parsePostPath (srcPostPath: string) {
    ...
    const regex = new RegExp(`post${separator}([^${separator}]+)${separator}([^${separator}]+)${separator}?(.*)?$`, 'i');
...
async function copyPosts (postsToCopy: Record<string, string[]>) {
  await Promise.all(Object.keys(postsToCopy).map(dirPath =>
    Promise.all(postsToCopy[dirPath].map(filename =>
...
function postDestinationPath (srcFilepath: string) {
  ...
  return withCwd('..', locale === DEFAULT_LOCALE ? DEFAULT_LOCALE_DIR : locale, 'post', postname, rest);
}

async function clearPosts () {
  await Promise.all(LOCALES.map(locale =>
    fsExtra.remove(withCwd('..', locale === DEFAULT_LOCALE ? DEFAULT_LOCALE_DIR : locale, 'post'))
  ));
}

async function filterOutDirs (fileTree: Record<string, string[]>) {
  const fileTreeCopy = {};
  await Promise.all(Object.keys(fileTree)
    .map(async dirPath => {
      fileTreeCopy[dirPath] = [];
      await Promise.all(fileTree[dirPath].map(async pathname => {
        if (await isFile(withCwd(dirPath, pathname))) fileTreeCopy[dirPath].push(pathname);
      }));
